...
import { EDateTagProps, EDay, EMonth, EYear } from '@/lib/typings/Date';
...
export function fromYears(s: DateTag, e: DateTag) {
  ...
  if (yearList.length === 1) {
    ...
    if(yearStart && yearEnd){
      ...
      const hashesStart = buildStrBin(yearStart.year.toString(2), 14);
  
      // calculate the nodes for the range start year
      const hashesEnd = buildStrBin(yearEnd.year.toString(2), 14);
  
      // calculate the nodes for the range end year
      const hashesStartSub = fromMonths(yearStart, null, hashesStart);
      const hashesEndSub = fromMonths(null, yearEnd, hashesEnd);
...
export function fromMonths(s: DateTag | null, e: DateTag | null, p: string) {
  ...
  if (list.length === 1 && s != null && e!=null) {
